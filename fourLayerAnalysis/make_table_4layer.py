...
cutFiles = ["cutflow_4layer_data.txt"]

nRows = 16
nTriggerablePaths = [64,160,160,160]
...
for row in range(nRows):
    ...
    if "Trigger" in selectionNames[row] and not "path" in selectionNames[row]: continue
    ...
    for i, cut in enumerate(cutArray[row]):
        ...
        nPaths = nTriggerablePaths[i]

        # if i not in [0,1]:
            # cut = round(cut,1)
            # cut = round(cut*1.0/nPaths,2)
        # else:
            # cut = round(cut*1.0/nPaths,2)
        
        # if i == 0: FOR BEAM ON DATA
