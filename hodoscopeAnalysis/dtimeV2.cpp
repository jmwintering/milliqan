...
void dtimeV2(){
	...
	int numRuns = dirs.size();
	cout << "Working on " << numRuns << " runs" << endl;
	...
	char name[20];
      ...
      sprintf(title,"hDt%d",i);
      hDt[i] = new TH1D(name,title,100,-10,1);
   	}
	...
	for(int iRun = 0; iRun < numRuns; iRun++){
		...
		TString dirT = dirs[iRun];
		string dirS = dirs[iRun].Data();
		...
		int nFiles = numberFiles("/store/user/llavezzo/" + dirs[iRun]);
		chain->Reset();

		for(int  iFileNum = 1; iFileNum < nFiles; ++iFileNum){
    		chain->Add("/store/user/llavezzo/" + dirT + Form("/UX5MilliQan_" + runTitle + ".%d_*" + "_v16.root", iFileNum));
  		}
		...
		for(int i = 0; i < chain->GetEntries(); i++){
			...
			if(current_time != previous_time) {
				...
				//cout << count[iRun] << endl;
				//cout << sync[iRun][count[iRun]] << endl;
				//cout << hour[iRun][count[iRun]] << endl;
