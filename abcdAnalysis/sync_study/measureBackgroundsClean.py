...
def measureBackgrounds(inputFile,blind,beamString,useSaved,nPEStrings,deltaTStrings,selections,slabs,signal,randN):
    ...
    # for iE in range(nEvents):
        ...
        compFile = 478
        compEvent = 428
        flag = tree.run == compRun and tree.file == compFile and tree.event == compEvent
        ...
        if flag: print('made it 1')

        if maxNPECorr/minNPECorr > 10: 
            continue

        if flag: print('made it 2')

        if flag:
            print('\n', panelPresent)
            print(maxDeltaT)
            print((abs(maxDeltaT/timingSel - 1) < 0.01))
            print(abs(abs(maxDeltaT/timingSel) - 1) < 0.01)
            ...
            # if True:
                if len(sels) > 0:
                    if tuple(sorted(list(chanSet))) in allPaths["Straight"]:
                        ...
                        pulseMaxFile = open("comp_straight.txt", "a+")
                        pulseMaxFile.write("{0} {1} {2} {3}\n".format(tree.run, tree.file, tree.event, maxDeltaT))
                        pulseMaxFile.close()

                        if minNPE > 2 and minNPE < 20:
                            pulseFile = open("comp_straight_region1.txt", "a+")
                            pulseFile.write("{0} {1} {2} {3}\n".format(tree.run, tree.file, tree.event, maxDeltaT))
                            pulseFile.close()
                        elif minNPE > 20:
                            pulseFile = open("comp_straight_region2.txt", "a+")
                            pulseFile.write("{0} {1} {2} {3}\n".format(tree.run, tree.file, tree.event, maxDeltaT))
                            pulseFile.close()
                    ...
                    if tuple(sorted(list(chanSet))) in allPaths["StraightPlusSlab"]:
                        ...
                        if minNPE > 5 and minNPE < 30:
                            pulseFile = open("comp_straight_region3.txt", "a+")
                            pulseFile.write("{0} {1} {2} {3}\n".format(tree.run, tree.file, tree.event, maxDeltaT))
                            pulseFile.close()
                        elif minNPE > 30:
                            pulseFile = open("comp_straight_region4.txt", "a+")
                            pulseFile.write("{0} {1} {2} {3}\n".format(tree.run, tree.file, tree.event, maxDeltaT))
                            pulseFile.close()
                    ...
                    if tuple(sorted(list(chanSet))) in allPaths["StraightPlusTwoOrMoreSlabs"]:
                        ...
                        pulseMaxFile = open("comp_straight_region5.txt", "a+")
                        pulseMaxFile.write("{0} {1} {2} {3}\n".format(tree.run, tree.file, tree.event, maxDeltaT))
                        pulseMaxFile.close()
                        # if maxNPE < 100:
                        #     nPassMaxNPE += 1
