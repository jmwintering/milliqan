...
)
{
	...
	TString inputFile = simulation ?  "/data/simulationData/"+masscharge+".root" : "/data/skims/allPhysicsAndTripleChannelSinceTS1_threeLayerHit_200303.root";
	...
	for(int i = 0; i < entries; i++){
		...
		for(int j = 0; j < (*v_ipulse).size(); j++){
			...
			if((*v_chan)[j] == 28 || (*v_chan)[j] == 20) continue;
		...
		for(int iPulse = 0; iPulse < (*v_sideband_RMS).size(); iPulse++){
			...
			if(iPulse == 28 || iPulse == 20) continue;
