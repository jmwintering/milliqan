...
void line(double t, std::vector<double> parFit, double &x, double &y, double &z)
{
  double p[4];
  // xz
...
std::vector<int> convertRawToPhysCh(unsigned int raw_ch, bool isHS)
{
  ...
  if(isHS)
  {
     ...
     {
       std::cout << "HS raw_ch = " << raw_ch << " is not correct. Enter a correct raw_ch." << std::endl;
     }
  }
  else
  {
     ...
     {
       std::cout << "TP raw_ch = " << raw_ch << " is not correct. Enter a correct raw_ch." << std::endl;
     }
  }
...
void lineInverted(double t, std::vector<double> parFit, double &x, double &y, double &z)
{
  double p[4];
  // xz
  //p[0] =  -1.*parFit.at(1)/parFit.at(0);
...
// track fit
std::vector<double> doFit(std::vector<int> *hs_data)
{
  // return this std::vector
  std::vector<double> parFit;
  parFit.clear();
  ...
  if(0)
  {
    std::cout << "Number of top vertical hits:   " << n_hs_topver << std::endl;
    std::cout << "Number of bot vertical hits:   " << n_hs_botver << std::endl;
    std::cout << "Number of top horizontal hits: " << n_hs_tophor << std::endl;
    std::cout << "Number of bot horizontal hits: " << n_hs_bothor << std::endl;
    std::cout << "Perform fit or not:            " << doTrackFit << std::endl;
  }
  ...
  for(unsigned int i=0; i<hs_data->size(); i++)
  {
    ...
    if(0)
    {
      std::cout << hs_data->at(i) << " :: " << phys_ch[0] << " " << phys_ch[1] << " " << phys_ch[2] << std::endl;
      std::cout << coord_ch[0] << " +/- " << coord_ch[3] << std::endl;
      std::cout << coord_ch[1] << " +/- " << coord_ch[4] << std::endl;
      std::cout << coord_ch[2] << " +/- " << coord_ch[5] << std::endl;
      std::cout << std::endl;
    }
...
//
void getPathLengthInverted(std::vector<double> parFit, std::vector<int> &chan, std::vector<double> &path_length)
{
...
void loadHSDataTree(TString hs_file_dir, TString hs_file, std::vector<hsData> &hsDataLoaded, int kind, bool preTS1=false)
{
  ...
  // std::cout << (Form("Loading %s HS file: %s/%s", kind_str(kind).Data(), hs_file_dir.Data(), hs_file.Data())) << std::endl;
...
std::vector<double> doCircleFit(std::vector<int> * hs_data)
{
  // return this std::vector
  std::vector<double> parFit;
  parFit.clear();
...
int checkIntervalOverlap(double min1Temp, double max1Temp, double min2Temp, double max2Temp) {
  ...
  bool point1 = min1 == max1;
  bool point2 = min2 == max2;
...
void getCirclePathLengthInverted(std::vector<double> parFit, std::vector<int> &chan, std::vector<double> &path_length, int runFit, int fileFit, int eventFit, string stringFit, bool& mathematicaRequired, bool weighted)
{
  bool testing = false;
  ...
  for(int i=0; i<2*ndiv; i++)
  {
    ...
    //std::cout << "{Ellipsoid[{" << x << ", " << y << ", " <<  z << "},{" << x_axis_length << ", " << y_axis_length << ",0}]}, " << std::endl;
    ...
    //   for(auto c:cols){
    //     for(auto r:rows){
    //       //std::cout << "l: "<< layer << " c: " << c << " r: " << r << std::endl;
    //       //std::cout << "ch: " << channelMap[layer][c][r] << std::endl;
    ...
    if(channelHit && !testing && axesInsideSquare < 2 && weighted){
      ...
      fout << runFit << " " << fileFit << " " << eventFit << " " << x << " " << y << " " << z << " " << x_axis_length << " " << y_axis_length << std::endl;
    }
...
std::vector<double> doCircleFitInverted(std::vector<int> * hs_data)
{
  // return this std::vector
  std::vector<double> parFit;
  parFit.clear();
...
void getCirclePathLengthInverted(std::vector<double> parFit, std::vector<int> &chan, std::vector<double> &path_length)
{
  ...
  // std::cout << "PARFIT: ";
  // for(unsigned int iP=0; iP<parFit.size(); ++iP) std::cout << parFit[iP] << " ";
  // std::cout << std::endl;
...
std::vector<double> convertBarChToPhysCoord(int barCh){
    std::vector<int> row3 = {0,1,6,7,2,3};
    std::vector<int> row2 = {24,25, 16,17, 22,23};
    std::vector<int> row1 = {8,9,12,13,4,5};
